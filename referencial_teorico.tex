...
A Web semântica é uma extensão da web atual em que a informação dada possui significado bem definido, permitindo que os computadores e pessoas possam trabalhar em cooperação \cite[p. 13]{wober}. Este novo formato da World Wide Web tem como visão ser tão capaz  quanto a World Wide Web (WWW) atual, mas consiste em dados legíveis por máquina, que possui o potencial para ter um maior impacto na busca na web.
%WWW repetido
...
%evitar o uso de termos fortes como deliberado
...
%o que signifca cola entre componentes? explicar logo a seguir o termo, não depois
...
O IronPython \cite{ironpython} é uma implementação da linguagem de programação Python que é executada no framework .NET e Silverlight. Suporta um shell interativo (como a maioria das implementações da linguagem Python) com completa compilação dinâmica. É bem integrado com o resto do framework .NET e torna todas as bibliotecas do .NET facilmente disponíveis para programadores Python, enquanto mantém a compatibilidade com a linguagem Python.
%você usa termos fortes demais, eu já entendi que python é perfeito
...
%coloca uma nota de rodapé para o matlab
...
%você traduziu esse trecho no translate?
...
\end{itemize}
%não entendi metade dos aspectos citados anteriormente
...
\begin{itemize}
	...
	\item Está se tornando rapidamente um padrão na Web, em que iniciativas como o JSON RPC \cite{json_rpc} e a pretensão do Twitter abandonar o XML para adotar o JSON \cite{twitter_deprecates_xml} provam isso.
\end{itemize}

%mais imagens seria interessante.
%quando tem código é bom explicar melhor o que o código faz
%