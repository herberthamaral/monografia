...
Uma preocupação constante no desenvolvimento do presente trabalho foi a procura por adoção de padrões abertos que sejam amplamente utilizados. Além do \gls{wpt}, outras duas alternativas foram pesquisadas: o \textit{Protocol Buffer} \cite{protobuf} e o JSON \cite{JSON}. Ambas as soluções foram descartadas pelos motivos citados anteriormente, ao passo que o \gls{wpt} se encaixou bem na lista de requisitos.
